//
//  YCTGetGoodsTypeListViewController.m
//  YCT
//
...
@interface YCTGetGoodsTypeListViewController ()<UITableViewDataSource, UITableViewDelegate>

@property (strong, nonatomic) YCTGetGoodsTypeListViewModel *viewModel;
@property (strong, nonatomic) YCTGetGoodsTypeListViewModel *nextLevelViewModel;

@property (strong, nonatomic) UITableView *tableView;

@property (weak, nonatomic) id<YCTGetGoodsTypeListViewControllerDelegate> delegate;
...
- (instancetype)initWitPid:(NSString *)pid
                 viewModel:(YCTGetGoodsTypeListViewModel *)viewModel
                cLocations:(nullable NSArray<YCTMintGetLocationModel *> *)cLocations
                  delegate:(id<YCTGetGoodsTypeListViewControllerDelegate>)delegate
NS_DESIGNATED_INITIALIZER;

@end

@implementation YCTGetGoodsTypeListViewController
...
- (void)bindViewModel {
    ...
    RACSignal *nlViewModelToastSubject = [[RACObserve(self, nextLevelViewModel) skip:1] flattenMap:^__kindof RACSignal * _Nullable(YCTGetGoodsTypeListViewModel * _Nullable value) {
        return value.toastSubject;
    }];
    
    RACSignal *nlViewModelLoadingSubject = [[RACObserve(self, nextLevelViewModel) skip:1] flattenMap:^__kindof RACSignal * _Nullable(YCTGetGoodsTypeListViewModel * _Nullable value) {
        return value.loadingSubject;
    }];
    
    RACSignal *nlViewModelLoadAllDataSubject = [[RACObserve(self, nextLevelViewModel) skip:1] flattenMap:^__kindof RACSignal * _Nullable(YCTGetGoodsTypeListViewModel * _Nullable value) {
...
- (void)didRequestNextLevelData {
    ...
    if (self.nextLevelViewModel.dataDict.count == 0) {
        ...
        if ([self.delegate respondsToSelector:@selector(goodsTypeDidSelectWithAll:lastPrev:last:)]) {
            [self.delegate goodsTypeDidSelectWithAll:self.cLocations.copy lastPrev:lastPrev last:self.cLocations.lastObject];
        }
    } else {
        YCTGetGoodsTypeListViewController *vc = [[YCTGetGoodsTypeListViewController alloc] initWitPid:self.currentSelectedModel.cid viewModel:self.nextLevelViewModel cLocations:self.cLocations delegate:self.delegate];
