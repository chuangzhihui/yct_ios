...
-(NSString*)firstPinYin
{
    ...
    if ([selfStr rangeOfString:@"重庆"].location != NSNotFound) {
        selfStr = [selfStr stringByReplacingOccurrencesOfString:@"重" withString:@"chong"];
    } else if ([selfStr rangeOfString:@"长"].location != NSNotFound) {
        selfStr = [selfStr stringByReplacingOccurrencesOfString:@"长" withString:@"chang"];
    } else if ([selfStr rangeOfString:@"厦"].location != NSNotFound) {
        selfStr = [selfStr stringByReplacingOccurrencesOfString:@"厦" withString:@"xia"];
    }
    
    if ([selfStr canBeConvertedToEncoding:NSASCIIStringEncoding]) {//it is english words
        return [selfStr substringToIndex:1];
    }

    unichar ch = [selfStr characterAtIndex:0];
    char firstPinYin = ch;
...
- (CGFloat)widthFromFont:(UIFont *)font
{
    ...
    return [self boundingRectWithSize:CGSizeMake(0, font.lineHeight) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading | NSStringDrawingUsesDeviceMetrics attributes:@{NSFontAttributeName : font} context:nil].size.width;
}
@end
